...
public class LeastUniqueDigits {
    static int leastDigits(int arr[],int k){
        ...
        // Create a map with keys as the array entries and values as their count
        for(int i=0;i<arr.length;i++){
            map.put(arr[i],map.getOrDefault(arr[i], 0)+1);
        }
        // Put them in a list and sort them using a custom comparator
        List<Map.Entry<Integer,Integer>> elements = new ArrayList<>(map.entrySet());
        elements.sort((a,b) -> a.getValue() - b.getValue());
        // Remove the least occuring elements first and remove them from the map
